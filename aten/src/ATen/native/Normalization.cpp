...
TORCH_META_FUNC(native_batch_norm)(const at::Tensor & input,
  ...
  if (input.device() == kCPU) {
    // TODO(whc) probably can't just * all the optionals here...
    checkBackend("batch_norm_cpu", {input, *weight, *bias, *running_mean, *running_var}, Backend::CPU);
    if (all_contiguous) {
      auto input_options = input.options();
      input_options.memory_format(at::MemoryFormat::Contiguous);
      // TODO(whc) is this part actually right?
      MetaBase::set_output(0, input.sizes(), input_options);
      MetaBase::set_output(1, {n_input}, input.options());
      MetaBase::set_output(2, {n_input}, input.options());
    } else {
    ...
    auto meanvar_options = input.options().dtype(
        at::toAccumulateType(input.scalar_type(), /*is_cuda=*/true));
    MetaBase::set_output(0, input.sizes(), input.options());
    MetaBase::set_output(1, {n_input}, meanvar_options);
    MetaBase::set_output(2, {n_input}, meanvar_options);
  }
