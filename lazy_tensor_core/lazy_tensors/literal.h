...
class Literal {
 public:
  ...
  lazy_tensors::Span<const NativeT> data(
    ...
    DCHECK(shape_index.empty()) << "Sub-literals not supported yet";
...
inline void Literal::PopulateR1(lazy_tensors::Span<const NativeT> values) {
  ...
  DCHECK_EQ(shape().rank(), 1);
  DCHECK_EQ(ShapeUtil::ElementsIn(shape()), values.size());
  DCHECK_EQ(shape().at_element_type(),
            c10::CppTypeToScalarType<NativeT>::value);
